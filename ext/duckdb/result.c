...
static VALUE to_ruby_obj_decimal_to_double(duckdb_result *result, idx_t col_idx, idx_t row_idx);
...
static VALUE duckdb_result__decimal_to_double(VALUE oDuckDBResult, VALUE row_idx, VALUE col_idx);
...
static VALUE to_ruby_obj_decimal_to_double(duckdb_result *result, idx_t col_idx, idx_t row_idx) {
    duckdb_decimal dval = duckdb_value_decimal(result, col_idx, row_idx);
    return DBL2NUM(duckdb_decimal_to_double(dval));
}
...
static VALUE duckdb_result__decimal_to_double(VALUE oDuckDBResult, VALUE row_idx, VALUE col_idx) {
    ...
    return to_ruby_obj_decimal_to_double(&(ctx->result), NUM2LL(col_idx), NUM2LL(row_idx));
}
...
void init_duckdb_result(void) {
    ...
    rb_define_private_method(cDuckDBResult, "_decimal_to_double", duckdb_result__decimal_to_double, 2);
